...
def unpack_tx_command(buf: memoryview) -> Tuple[np.ndarray, np.ndarray, int, int]:
    ...
    position_out = np.array(
        struct.unpack_from('<6i', buf, layout.POSITION_OUT_OFFSET),
        dtype=np.int32
    )
    speed_out = np.array(
        struct.unpack_from('<6d', buf, layout.SPEED_OUT_OFFSET),
        dtype=np.float64
    )
...
def pack_rx_frame(
    ...
    buf[layout.PAYLOAD_OFFSET:layout.PAYLOAD_OFFSET + layout.PAYLOAD_SIZE] = payload[:52]
...
def unpack_ik_request(buf: memoryview) -> Tuple[np.ndarray, np.ndarray, int, int]:
    ...
    T_flat = np.array(
        struct.unpack_from('<16d', buf, layout.T_FLAT_OFFSET),
        dtype=np.float64
    )
...
def pack_ik_response(
    ...
    buf[layout.CART_EN_WRF_OFFSET:layout.CART_EN_WRF_OFFSET + 12] = bytes(cart_en_wrf[:12])
    buf[layout.CART_EN_TRF_OFFSET:layout.CART_EN_TRF_OFFSET + 12] = bytes(cart_en_trf[:12])
    struct.pack_into('<Q', buf, layout.RESP_SEQ_OFFSET, resp_seq)


def unpack_ik_response(buf: memoryview) -> Tuple[np.ndarray, np.ndarray, np.ndarray, int]:
    """Unpack IK response from output shared memory buffer."""
    layout = IKOutputLayout
    joint_en = np.frombuffer(
        buf[layout.JOINT_EN_OFFSET:layout.JOINT_EN_OFFSET + 12],
        dtype=np.uint8
    ).copy()
    cart_en_wrf = np.frombuffer(
        buf[layout.CART_EN_WRF_OFFSET:layout.CART_EN_WRF_OFFSET + 12],
        dtype=np.uint8
    ).copy()
    cart_en_trf = np.frombuffer(
        buf[layout.CART_EN_TRF_OFFSET:layout.CART_EN_TRF_OFFSET + 12],
        dtype=np.uint8
    ).copy()
